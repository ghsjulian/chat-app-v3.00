...
const useChatStore = create((set, get) => ({
  ...
  loadMoreMessages: async () => {
    ...
    try {
      ...
      console.log("Error In Load More Message", error);
    }
  },
  loadMoreUsers: async () => {
    const { chatUsers } = get();
  ...
  updateStatus: (data) => {
    set((state) => ({
      currentChats: state.currentChats.map((chat) =>
        chat.tempId === data.msgId ? { ...chat, seen: data.status } : chat
      ),
    }));
  },
  setStatus: () => {
    ...
    console.log("Delivery Status Users : ", users);
  },
  setSeenStatus: (data) => {
    const socketState = useSocket.getState();
